  ...
  return (
    <div>
      {!editedModels && !editedForms && <form onSubmit={handleSubmit} className="form">
        ...
        <div className='row'>
          <button className="col btn btn-danger" >Отмена</button>
          <button onClick={() => autoGenFormMetadata()} className="col btn btn-outlined">Сгенерировать метаданные форм</button>
          <button onClick={() => genCode()} className="col btn btn-outlined">Сгенерировать код</button>
          <button onClick={() => deployProject()} className="col btn btn-outlined">Развернуть проект</button>
          <button className="col btn btn-success" type="submit">Сохранить</button>
